...
\begin{center}
	...
	\draw[<->]
		...
		(0.5*\w*0.95, 0.6*\a);
	\draw[<->] (0,0) -- node[right] {$h_x$} (0.0, 0.95*\hx);
	\draw[<->] (0,0) -- node[below] {$h_y$} (0.95*\hy, 0);
\end{tikzpicture}
\end{center}
\caption{}%
\label{fig:unitcell}
...
Enforcing a wave vector $k\hat y$ and running COMSOL to find the eigenmodes with their
...
\begin{figure}[htpb]
	...
	\caption{chapters/theory/banddiagram.pdf}
